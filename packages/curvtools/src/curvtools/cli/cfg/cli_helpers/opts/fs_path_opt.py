...
class FsPathType(_SystemPath):
    ...
    (begins with a separator) in which case we donly do variable expansion.
    """
    _flavour = _SystemPath._flavour
    def __new__(cls, path: str, ctx: click.Context):
        # Expand special variables first, then construct a concrete absolute Path
        s = expand_curv_root_dir_vars(path, ctx)
        s = expand_build_dir_vars(s, ctx)
        resolved: str = str(Path(s).expanduser().absolute())
